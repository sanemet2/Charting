...
1. **Service Layer** (`Program/service/chart_library.py`, storage module, tests)
   - Implement per `04_service_layer_plan.md`.
   - Add logging, atomic writes, and unit tests before exposing APIs to Dash.
2. **Dash Stores & Adapters** (`Program/presentation/library_state.py`, layout IDs, stores)
   - Follow `05_dash_state_plan.md`; ensure incremental store updates and preloaded Program.data.
3. **UI Scaffold** (`Program/presentation/layout.py`, `ids.py`)
   - Build tree-based panel described in `06_ui_plan.md` with placeholders for future search/tagging.
4. **Callbacks** (`Program/presentation/app_dash.py`)
